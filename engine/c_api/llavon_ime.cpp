...
void lv_engine_options_init(lv_engine_options* options) {
    ...
    options->threads = 8;
    options->gpu_layers = -2;
    options->idle_timeout_seconds = 1800;
...
int lv_engine_create(const lv_engine_options* options, const lv_host* host, lv_engine** out) {
    ...
    if (options->table_path == nullptr || options->phrase_overrides_path == nullptr) return -1;
    try {
        auto engine = std::make_unique<lv_engine>(*host);
        EngineOptions engine_options;
        engine_options.table_path = options->table_path;
        engine_options.phrase_overrides_path = options->phrase_overrides_path;
        ...
        if (options->config_json != nullptr && options->config_json[0] != '\0') {
            ...
            engine_options.config = default_config();
        }
        ...
        if (options->model_path != nullptr) transport.model_path = options->model_path;
        if (options->tables_dir != nullptr) transport.tables_dir = options->tables_dir;
        transport.context_length = options->context_length;
        transport.threads = options->threads;
        transport.gpu_layers = options->gpu_layers;
        transport.idle_timeout_seconds = options->idle_timeout_seconds;
