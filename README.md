...
服務。安裝程式會把輸入來源註冊、重新啟動輸入法，並在 macOS 15 及更早的版本
把來源加入啟用清單；升級後原本就在輸入選單裡的會繼續保留，不必登出。
macOS 26（Tahoe）把第三方輸入來源的啟用狀態存在受保護的系統 store，只有
「系統設定」能寫入，所以第一次安裝要自己到「系統設定 › 鍵盤 › 輸入方式」
按「+」加入（或在安裝後登出再登入一次）。
