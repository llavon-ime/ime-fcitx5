...
「安裝／更新 LoRA Trainer」會偵測顯示卡：AMD 會安裝不含 LibTorch 的 ROCm 版、
NVIDIA 會安裝不含 LibTorch 的 CUDA 版，並各自下載官方 LibTorch 到
`~/.cache/llavon-lora`（分別約 9.4 GB 與 3.9 GB，只下載一次）；其他機器安裝
CPU 版（macOS 產物已內建 Metal）。要用自己建置的 trainer 時，把
